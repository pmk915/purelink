...
- Run id: `20260711-181542-auto-block_aware`
- Created at: `2026-07-11T18:15:42.782010+00:00`
- Commit: `d9c4ed8`
...
|---|---:|---:|---:|---:|---:|---:|---:|
| `entity_attribute` | 8 | 8 / 8 (100.0%) | 4 / 8 (50.0%) | 6 / 8 (75.0%) | 8 / 8 (100.0%) | 8 / 8 (100.0%) | 24.6% (n=8) |
| `entity_definition` | 8 | 8 / 8 (100.0%) | 2 / 8 (25.0%) | 5 / 8 (62.5%) | 8 / 8 (100.0%) | 8 / 8 (100.0%) | 33.1% (n=8) |
| `entity_reason` | 6 | 6 / 6 (100.0%) | 0 / 6 (0.0%) | 3 / 6 (50.0%) | 6 / 6 (100.0%) | 6 / 6 (100.0%) | 12.5% (n=6) |
| `entity_relation` | 8 | 8 / 8 (100.0%) | 2 / 8 (25.0%) | 3 / 8 (37.5%) | 8 / 8 (100.0%) | 8 / 8 (100.0%) | 19.8% (n=8) |
| `no_answer` | 5 | n/a | n/a | n/a | 5 / 5 (100.0%) | 0 / 5 (0.0%) | 0.0% (n=1) |
| `overview` | 5 | 3 / 5 (60.0%) | 3 / 5 (60.0%) | 2 / 5 (40.0%) | 5 / 5 (100.0%) | 5 / 5 (100.0%) | 5.7% (n=5) |
| `technical` | 10 | 9 / 10 (90.0%) | 6 / 10 (60.0%) | 5 / 10 (50.0%) | 10 / 10 (100.0%) | 10 / 10 (100.0%) | 19.4% (n=9) |

## 4. Metrics by Selected Mode

| Group | Cases | retrieval_hit | citation_hit | expected_evidence_hit | router_accuracy | evidence-gate answerability | evidence_precision |
|---|---:|---:|---:|---:|---:|---:|---:|
| `chunk_only` | 30 | 24 / 25 (96.0%) | 8 / 25 (32.0%) | 16 / 25 (64.0%) | 30 / 30 (100.0%) | 25 / 30 (83.3%) | 25.3% (n=26) |
| `graph_vector_mix` | 8 | 8 / 8 (100.0%) | 2 / 8 (25.0%) | 3 / 8 (37.5%) | 8 / 8 (100.0%) | 8 / 8 (100.0%) | 19.8% (n=8) |
| `hybrid_text` | 7 | 7 / 7 (100.0%) | 4 / 7 (57.1%) | 3 / 7 (42.9%) | 7 / 7 (100.0%) | 7 / 7 (100.0%) | 9.0% (n=6) |
| `overview` | 5 | 3 / 5 (60.0%) | 3 / 5 (60.0%) | 2 / 5 (40.0%) | 5 / 5 (100.0%) | 5 / 5 (100.0%) | 5.7% (n=5) |
...
| `no_answer_ceo` | n/a | n/a | n/a | True | false | n/a | unexpected_answerable |
...
| `def_fastapi_dependency` | FastAPI dependency 是什么？ | declare reusable requirements, dependency is usually a callable | FastAPI includes dependency information in OpenAPI when it affects parameters or security. This allows generated API doc; A dependency can depend on another dependency. FastAPI resolves this tree and reuses results where appropriate during on | `chunk_only` | expected_evidence_not_selected, citation_missing |
...
| `def_document_block` | DocumentBlock 是什么？ | DocumentBlock records preserve structure, source spans | PureLink validates uploads before processing. Supported local demo text formats include Markdown-like text, Markdown, PD; PureLink supports fixed and blockaware chunk strategies. Fixed chunking creates character-based chunks. Block-aware chun | `chunk_only` | citation_missing |
...
| `attr_deepseek_config` | DeepSeek API 配置位于哪里？ | API paths, environment variables, code-like tokens | Source basis: Project Gutenberg edition of Alice's Adventures in Wonderland  Adaptation: concise character facts for det; hybridtext helps with API paths, environment variables, code-like tokens, file names, commands, and other exact text. It | `chunk_only` | citation_missing |
...
| `reason_postgres_mvcc` | PostgreSQL 为什么使用 MVCC？ | readers and writers can often operate without blocking, preserve consistency | Source basis: PostgreSQL official documentation, MVCC and locking  Adaptation: concise paraphrase for deterministic Pure; 角色：产品经理  办公地点：Beijing  负责：知识库工作台和用户研究  隶属：Product Group  Carol Wang defines workspace workflows and collects user feedba | `chunk_only` | citation_missing |
...
| `rel_mvcc_locks` | MVCC 和显式锁是什么关系？ | MVCC reduces many read-write conflicts, explicit locks still exist | Source basis: PostgreSQL official documentation, MVCC and locking  Adaptation: concise paraphrase for deterministic Pure; PostgreSQL uses multiversion concurrency control, or MVCC, so readers and writers can often operate without blocking eac | `graph_vector_mix` | expected_evidence_not_selected, citation_missing |
...
| `no_answer_ceo` | Acme 的 CEO 是谁？ | - | Deadlocks can occur when transactions wait on each other in a cycle. PostgreSQL detects deadlocks and cancels one transa; PureLink validates uploads before processing. Supported local demo text formats include Markdown-like text, Markdown, PD | `chunk_only` | unexpected_answerable |
