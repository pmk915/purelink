...
- 前端工作区里的“开始处理”默认直接调用同步 `parse -> chunk -> embed` 接口，适合本地手动联调，不要求必须先启动 Go worker
- `document_tasks` + Go worker 仍然完整保留，主要用于脚本化 E2E、worker 验证和后续异步扩展

这意味着：

- 你在浏览器里手动上传 `.txt` / `.md` 后，可以直接点击“开始处理”完成最小闭环
...
- 当前前端工作区里的“开始处理”默认改成直接调用同步 `parse -> chunk -> embed` 接口，适合本地手动联调
