...
本文件由旧版 `README.md` 迁移而来，保留了项目从后端骨架到团队协作、文档处理链、Go worker、检索与最小问答的详细开发记录。

如果你是第一次进入仓库，先看 [README.md](README.md)。
如果你想看里程碑规划，再看 [PLAN.md](PLAN.md)。
...
截至当前版本，PureLink 有两条都有效的文档处理路径：

- 前端工作区里的“开始处理”默认直接调用统一 `/process` 接口，当前适用于 `.txt / .md / .pdf / .docx`，会写入数据库 `DocumentChunk`，并把文档推进到 `ready / failed`
- `ready` 文档会直接基于数据库 chunk 进入最小检索与问答，不再强依赖 embedding 产物才能可用
- 旧的 `document_tasks + Go worker + parse / chunk / embed / index` 链路仍然完整保留，主要用于脚本化 E2E、worker 验证和后续异步扩展

这意味着：

- 你在浏览器里手动上传 `.txt / .md / .pdf / .docx` 后，可以直接点击“开始处理”完成最小闭环
- 你在跑 `make smoke`、`make e2e`、`scripts/e2e/*.sh` 时，仍然要确保 `db / api / worker` 这套环境已经拉起

## 2026-04-23：M13 基础版完成与真实环境验证

这一轮完成的是文档处理链路的工程化基线，不再只是“接口直接跑函数”，而是引入最小可追踪任务流。

本轮实际完成的重点包括：

- 新增 `processing_jobs` 表与 `ProcessingJob` 模型，作为统一文档处理任务记录
- `/process` 已改为：
  - 先创建 `ProcessingJob`
  - 再由 Python inline worker 执行统一文档处理
  - 过程中记录步骤、触发者、尝试次数、失败原因
- 已新增 processing job 查询能力：
  - 按文档列出任务
  - 按 job id 查看任务详情
- 已新增：
  - `retry-process`
  - `reprocess`
- `DocumentRead` 现在会带出最近一次 processing job 的最小信息，便于前端和调试侧观察
- 已补充自动化测试，覆盖：
  - `/process` 成功创建并完成 job
  - 失败后进入 `failed`
  - 恢复源文件后 `retry-process` 成功
  - 已处理文档 `reprocess` 后替换 chunk
- 本地测试已通过：
  - `51 passed`

这一轮还完成了一次基于 Docker + PostgreSQL 的真实联调验证，验证了：

- 文档 `/process` 后进入 `ready`
- processing job 状态可查询
- `reprocess` 成功后，检索能读到新的 chunk 内容
- 人为删除源文件后，`/process` 会返回 `400` 和明确错误信息
- 恢复源文件后，`retry-process` 成功，job 历史顺序和状态正确

在真实 PostgreSQL 验证过程中，额外发现并修复了一个 SQLite 测试里不会暴露的问题：

- `documents.processing_status` 列仍然沿用旧长度，写入 `processing` 时会触发数据库截断并导致 `/process` 返回 `500`
- 已新增迁移 `20260423_0008_expand_document_processing_status_length.py` 修复该问题
- `api` 容器重建后，启动日志已确认执行这条迁移

当前需要记住的运行口径：

- 后端代码有变更时，如果你是通过 Docker 联调，不要只执行 `docker compose up -d api`
- 正确方式是：
  - `docker compose up -d --build api`
- 否则容器里仍然会运行旧镜像，容易出现“代码改了但接口还是旧行为”的假象

## 最近一轮开发收口

这一轮不是继续大规模扩业务，而是把“可演示、可联调、可公开展示”的部分补齐。

本轮实际完成的重点包括：

- 新增并落地了 `frontend/` 下的 Next.js 前端 MVP，覆盖登录、注册、工作台、个人知识库、团队、审核、问答和会话查看
- 前端补了中英文切换能力，当前支持在认证页和工作区页面切换 `EN / 中文`
- 首页和主导航从“演示导向”改为“工作台导向”，移除了明显的 `MVP / 开发中 / 下一轮再做` 之类的开发者文案
- 上传链路做过一轮浏览器端排查和修正，前端会打印更清楚的请求与错误上下文，方便定位 `Failed to fetch`、CORS、URL 配置等问题
- 文档“开始处理”卡住的问题已经定位并修复：根因是前端默认调用的是 `*-tasks` 入队接口，而不是立即执行的同步处理接口；在本地未启动 worker 时，这会表现为一直处于“处理中”
- 当前前端工作区里的“开始处理”默认改成直接调用统一 `/process` 接口，适合本地手动联调
- `document_tasks` + Go worker 这条异步链路仍然完整保留，用于脚本化 E2E、worker 验证和后续异步扩展
- 后端补了 parse / chunk / embed 关键步骤日志，日志中会输出 `document_id`、源文件路径、解析输出路径、chunk 数量和索引输出路径，便于直接从服务端定位处理卡点
- chunk 失败场景增加了 `processing_status = failed` 的回写，避免文档长期停留在中间态
- 增加了针对失败状态回写的回归测试
- 补齐并整理了公开展示和交付用文档：双语 `README.md`、详细 `DEVELOPMENT_LOG.md`、前后端联调说明 `test.md`、前端运行说明 `frontend/README.md`
- Makefile、Docker Compose、Bash E2E、GitHub Actions 已经形成一套可直接运行的工程化收口链路

这一轮之后，PureLink 的实际状态已经从“后端里程碑堆叠”进入到“全链路原型可运行 + 前端可演示 + 工程化可验证”的阶段。
...
- 文档上传与提交记录可用
- 团队文档审核流可用
- `.txt / .md / .pdf / .docx` 统一 `/process` 最小闭环可用
- `ProcessingJob` 基线与 legacy `document_tasks` 任务机制可用
...
- 团队待审核文档列表、审核通过与拒绝
- 个人文档与已审核团队文档的最小解析接口
- `.txt / .md / .pdf / .docx` 的统一 `/process` 处理接口
- `ProcessingJob` 模型、processing job 查询接口、`retry-process / reprocess`
- 解析任务创建与任务状态查询
- Go worker 可轮询 `pending parse` / `pending chunk` / `pending embed` / `pending index` 任务并推进状态
- 个人与团队文档的最小 chunk 接口
- 个人与团队知识库的最小 embedding / retrieval 接口
- 最小 ask 接口、citation 返回和问答会话持久化
- `frontend/` 下的前端 MVP 与中英文切换
- `scripts/e2e/` Bash 全流程脚本
- Makefile、Docker Compose、CI / smoke workflow
...
├── frontend/           # Next.js 前端 MVP
│   ├── app/            # App Router 页面
│   ├── components/     # 页面组件与 UI 组件
│   ├── hooks/          # 前端数据获取与状态 hooks
│   ├── locales/        # 中英文文案
│   └── README.md       # 前端本地启动说明
├── logs/               # 本地日志目录
├── scripts/            # 开发辅助脚本目录
...
当前仓库已经不是单纯的后端骨架，而是一个“后端 + 前端 MVP”的可运行原型。现阶段启动目标如下：

- 启动 PostgreSQL、FastAPI 服务和 Go worker
- 启动 Next.js 前端
- 通过前端页面完成注册、登录、创建知识库、上传、处理、检索和问答
- 访问 API 文档和健康检查接口确认后端可用
...
如果你要真正看到完整产品界面，还需要额外启动 `frontend/` 下的 Next.js 前端。当前仓库的 `docker-compose.yml` 只编排后端服务栈，不包含前端容器。
...
- Frontend API Base：`http://127.0.0.1:8000/api/v1`
...
如果你的后端不是运行在 `127.0.0.1:8000`，需要修改 `frontend/.env.local` 中的：
...
如果你的目标是“自己打开页面调试整个项目”，推荐按下面顺序启动：
...
如果你想验证“服务、数据库、文档处理链”都通了，最短路径如下：

1. 访问 `GET /api/v1/health`，确认 API 正常
2. 启动前端并登录 `http://127.0.0.1:3000`
3. 创建一个个人知识库
4. 上传一个 `.txt` 或 `.md` 文档
5. 在页面里点击“开始处理”，等待文档进入可搜索状态
6. 调用 retrieval 或 ask，确认能返回结果

如果你想额外验证 worker 路径，再执行：

1. 保持 `db / api / worker` 全部启动
2. 运行 `make smoke` 或 `make e2e`
3. 或直接创建 `parse-task`、`chunk-task`、`embed-task`，再查询 `GET /api/v1/document-tasks/{task_id}`
4. 确认任务进入 `succeeded`，并检查 `data/parsed/`、`data/chunks/`、`data/vector_store/`
...
- 如果你是手动点前端页面做联调，worker 不是必须条件
- 如果你运行 `04_worker_flow.sh`、`make smoke`、`make e2e`，则仍然要求 worker 正在运行
...
- 如果你想保留环境便于手动检查容器日志，可执行 `KEEP_STACK_UP=1 make smoke` 或 `KEEP_STACK_UP=1 make e2e`
